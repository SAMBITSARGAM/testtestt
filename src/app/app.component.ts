...
export class AppComponent implements OnInit {
    ...
    async ngOnInit() {
        this._httpClient
            ...
            .pipe(
                finalize(async () => {
                    const config = JSON.parse(sessionStorage.getItem('musicfy.config'));
                    await this._datastoreLoaderService.load(config);

                    this.walletAddress = this._walletService.getAddress();
                    localStorage.removeItem('musicfy.playlists');
                    localStorage.removeItem('musicfy.likedSongs');
            ...
            .subscribe(
                (res) => {
                    ...
                    sessionStorage.setItem('musicfy.config', JSON.stringify(res));
                },
                (error) => {
                    console.log('load config from angular env', environment);
                    sessionStorage.setItem('musicfy.config', JSON.stringify(environment));
                }
            );
    }
    ...
    async loadPlaylists() {
        ...
        localStorage.setItem('musicfy.playlists', JSON.stringify(this.playlists));
        this._eventsService.playlistLoaded$.next(this.playlists);
    }

    loadLikedSongs() {
        var myLikedSongs = DatastoreService.getLikedSongs(this.walletAddress);
        localStorage.setItem('musicfy.likedSongs', JSON.stringify(myLikedSongs));
    }
}
